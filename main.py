...
class Runner:
    ...
    def model_choise(self):
        assert self.args.model_name in ["EVA", "MCLEA", "MSNEA", "MEAformer", "OTMEA"]
        if self.args.model_name == "OTMEA":
            self.model = OTMEA(self.KGs, self.args)
    ...
    def il_for_ea(self):
        with torch.no_grad():
            if self.args.model_name in ["OTMEA"]:
