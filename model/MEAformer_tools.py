...
from .sinkhorn_cal import sinkhorn
import pdb
...
class MformerFusion(nn.Module):
    def __init__(self, args, modal_num, with_weight=1):
        ...
        gph_dim = self.args.char_dim
        rel_dim = self.args.attr_dim
        # cost matix
        ...
        if self.args.use_surface:
            self.weight = nn.Parameter(torch.ones((6, 1)),
                                   requires_grad=True)
        else:
            self.weight = nn.Parameter(torch.ones((4, 1)),
                                   requires_grad=True)
        self.mats_attr = nn.Parameter(torch.Tensor(attr_dim, gph_dim), requires_grad=True)
        nn.init.xavier_uniform_(self.mats_attr)

        self.mats_img = nn.Parameter(torch.Tensor(img_dim, gph_dim), requires_grad=True)
        nn.init.xavier_uniform_(self.mats_img)
        self.mats_name = nn.Parameter(torch.Tensor(name_dim, gph_dim), requires_grad=True)
        nn.init.xavier_uniform_(self.mats_name)
        self.mats_char = nn.Parameter(torch.Tensor(char_dim, gph_dim), requires_grad=True)
        nn.init.xavier_uniform_(self.mats_char)
        self.mats_rel = nn.Parameter(torch.Tensor(rel_dim, gph_dim), requires_grad=True)
        nn.init.xavier_uniform_(self.mats_rel)
        self.type_id = torch.tensor([0, 1, 2, 3, 4, 5]).cuda()

        alp = 0.5
        self.alp = nn.Parameter(torch.tensor(alp), requires_grad=True)
    ...
    def forward(self, embs):
        ...
        weight_norm_tmp = F.softmax(self.weight, dim=0)
       
        matrix1 = self.cal_ot(embs[1], embs[3], self.mats_attr)
        embs[1] = embs[1].mm(matrix1)
        matrix2 = self.cal_ot(embs[0], embs[3], self.mats_img)
        embs[0] = embs[0].mm(matrix2)
        matrix5 = self.cal_ot(embs[2], embs[3], self.mats_rel)
        embs[2] = embs[2].mm(matrix5)
        if self.args.use_surface == 1 :
            matrix4 = self.cal_ot(embs[4], embs[3], self.mats_name)
